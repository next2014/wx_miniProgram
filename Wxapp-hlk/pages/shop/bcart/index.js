...
Page({
  ...
  onShow: function (options) {
    ...
    that.loaddata();
    var select_arr = [];
    for (var j = 0; j < that.data.cart_list.length; j++) {
      select_arr[j] = 'error';
    }
    that.setData({ select_arr: select_arr, all_goods_price: 0, all_goods_number: 0, this_check_val: [], all_is_checked:false})
  },
  initgetCartListData: function (data) {
    var that = this
    var price = 0
    var num = 0
    var this_check_val = that.data.this_check_val
    for (var i = 0; i < this_check_val.length; i++) {
      price += that.data.cart_list[this_check_val[i]].goods_number * that.data.cart_list[this_check_val[i]].goods_info.price_info.price
      num += that.data.cart_list[this_check_val[i]].goods_number * 1
    }
    that.setData({
      cart_list: data,
      glo_is_load: false,
  ...
  all_checkboxChange: function (e) {
    ...
    if (e.detail.value[0] == 1) {
      ...
      that.setData({ select_arr: select_arr })
    } else {
      isallcheck = false
      that.setData({
        all_goods_price: 0,
  ...
  single_checkboxChange: function (e) {
    ...
    if (c_length >= g_length) {
      ...
      for(var j = 0; j < that.data.cart_list.length; j++){
        select_arr[j] = 'error';
        for(var v = 0; v < that.data.this_check_val.length; v++){
          if (j == that.data.this_check_val[v]){
            select_arr[j] = that.data.this_check_val[v];
          }
        }
      }
      that.setData({ select_arr: select_arr})
  ...
  bind_cart_number_jian: function (e) {
    var that = this
    if (that.data.complate == false) return;
    that.setData({complate: false})
    var this_cart_id = e.currentTarget.id
    var datas = that.data.cart_list
    for (var i = 0; i < datas.length; i++) {
      if (datas[i].id == this_cart_id) {
        var price = that.data.all_goods_price * 1
        var num = that.data.all_goods_number * 1
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm == true) {
          ...
          that.setData({ select_arr: '', all_goods_price: 0, all_goods_number: 0, this_check_val: []})
        }
      }
    })
  },
  ...
  loaddata: function () {
    var that = this;
    requestUtil.get(_DuoguanData.duoguan_host_api_url + '/index.php?s=/addon/DuoguanShop/DuoguanShopApi/bottomshopList', { version: 108 }, (info) => {
      console.log(info)
      that.setData({ index_set: info })
    });
  },
